...
/**
 * This class is a seek bar preference displaying string labels with associated integer values. NOTE
 ...
 */
public class ChoiceSeekBarPreference extends Preference implements LabelSeekBar.OnProgressListener {
     ...
     */
    public ChoiceSeekBarPreference(@NonNull Context context, @Nullable AttributeSet attributeSet) throws IllegalArgumentException {
        ...
        try {
            ...
            if ((id = typedArray.getResourceId(R.styleable.ChoiceSeekBar_choiceValues, 0)) != 0) {
                _valueArray = context.getResources().getIntArray(id);
            } else {
                throw new IllegalArgumentException(getContext().getString(R.string.generic_seek_bar_value_array_error));
            }
            _labelArray = typedArray.getTextArray(R.styleable.ChoiceSeekBar_choiceLabels);
    ...
    public void onBindViewHolder(@NonNull PreferenceViewHolder preferenceViewHolder) throws IllegalArgumentException {
        ...
        int index = -1, value = getPersistedInt(_defaultValue);
        // the next statement is crucial because, if the "preference view holder" isn't set
        // to be NOT recyclable, all "choice seek bar preference" instances will share the same
        // "choice seek bar" instance
    ...
    protected void onSetInitialValue(Object defaultValue) {
        ...
        setValue(getPersistedInt(Integer.parseInt(defaultValue.toString())));
    }
