...
public class CheckBoxPreference extends androidx.preference.CheckBoxPreference {
    ...
    public CheckBoxPreference(@NonNull Context context, @Nullable AttributeSet attributeSet) {
        ...
        try (TypedArray typedArray = context.obtainStyledAttributes(attributeSet, R.styleable.CheckBoxPreference, 0, 0)) {
            _isCheckedDefault = typedArray.getBoolean(R.styleable.CheckBoxPreference_displayPlayerDefaultValue, false);
            if ((_checkedId = typedArray.getResourceId(R.styleable.CheckBoxPreference_checkedId, -1)) < 0) {
                throw new RuntimeException(getContext().getString(R.string.check_box_preference_mandatory_error, "checkedId"));
            }
            if ((_uncheckedId = typedArray.getResourceId(R.styleable.CheckBoxPreference_uncheckedId, -1)) < 0) {
